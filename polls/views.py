...
def index(request):
    ...
    while page_nr <= pages:
        ...
        r = requests.get(
            'https://999.md/ro/list/transport/cars?applied=1&eo=20&view_type=short&r_6_2_unit=eur&ef=260&ef=6&ef=1&ef=7&hide_duplicates=yes&o_1_21_20=117&r_6_2_to=&r_6_2_from=',
            data=getParams)
    ...
    for year in range(startYear, endYear + 1):
        ...
        for itemList in itemsList:
            if itemList[0] == year:
                ...
                # set average price per year
                groupedByYears[year - startYear][2] += round(itemList[1])
                # update min price
                if groupedByYears[year - startYear][3] > itemList[1]:
    ...
    response = makeHTMLResponse(groupedByYears)
